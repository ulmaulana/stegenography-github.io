...
function hideMessageInImageDWT(canvas, ctx, img, message) {
    ...
    if (fullBinary.length > blocks.length) {
        showError('Message is too long for this image!');
        return;
    }
...
function extractMessageFromImageDWT(canvas, ctx, img) {
    ...
    for (let i = 0; i < 32; i++) {
        if (i >= blocks.length) {
            throw new Error('Image does not contain valid steganography message');
        }
        const dwtBlock = dwt2D(blocks[i]);
    ...
    if (message.length === 0) {
        throw new Error('Could not extract valid message');
    }
